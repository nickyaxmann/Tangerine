class Assessment extends Backbone.Model
  ...
  updateFromServer: ( dKey = @id.substr(-5,5)) =>
    ...
    $.ajax "http://#{Tangerine.config.address.cloud.host}:#{Tangerine.config.address.port}/#{Tangerine.config.address.cloud.dbName}/_design/#{Tangerine.config.address.designDoc}/_view/byDKey",
      type: "POST"
      dataType: "jsonp"
      data: keys: dKeys
      success: (data) =>
